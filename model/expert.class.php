...
class expertmodel {
    ...
    function get_solves($start = 0, $limit = 20) {
        $solvelist = array();
        $query = $this->db->query("SELECT a.qid,a.title FROM " . DB_TABLEPRE . "answer  as a ,`" . DB_TABLEPRE . "user` as f WHERE a.authorid=f.uid ORDER BY a.time DESC LIMIT $start ,$limit");
